    ...
    <div className="space-y-6">
        ...
        <div className="space-y-2.5">
          {['AC', 'Non-AC', 'Sleeper', 'Semi-Sleeper'].map((type) => (
            <label key={type} className="flex items-center space-x-3 cursor-pointer select-none">
              ...
              />
              <span className="text-xs text-slate-650 font-semibold">{type}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-1">
          ...
          />
          <div className="flex justify-between text-[11px] text-slate-450 font-bold mt-2">
            <span>Rs. 500</span>
          ...
          {[
            ...
            <label key={slot.name} className="flex items-center space-x-3 cursor-pointer select-none">
              <input
                ...
                className="w-4.5 h-4.5 rounded text-indigo-600 focus:ring-indigo-500/30 border-slate-300"
              />
              <span className="text-xs text-slate-650 font-semibold">{slot.label}</span>
            </label>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 to-indigo-950 text-white rounded-3xl p-6 md:p-8 mb-8 shadow-xl border border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-fade-in-up">
        ...
        <div className="flex items-center space-x-2.5 w-full md:w-auto relative z-10 mt-4 md:mt-0">
          <button
            onClick={() => setShowMobileFilters(true)}
            className="lg:hidden flex-grow md:flex-none text-xs font-extrabold text-slate-800 bg-white border border-slate-200/80 px-5 py-3.5 rounded-2xl flex items-center justify-center space-x-1.5 shadow-sm hover:bg-slate-50 transition-all duration-300 active:scale-95 cursor-pointer"
          >
            <HiOutlineAdjustments className="w-4.5 h-4.5 text-indigo-500" />
            <span>Filters</span>
          </button>
          <Link 
            to="/" 
            className="flex-grow md:flex-none text-xs font-extrabold text-white bg-indigo-600 hover:bg-indigo-700 px-5 py-3.5 rounded-2xl flex items-center justify-center space-x-1.5 shadow-md shadow-indigo-600/15 hover:shadow-indigo-600/35 transition-all duration-300 active:scale-95 text-center"
          >
      ...
      {showMobileFilters && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          ></div>
          <div className="fixed inset-y-0 left-0 max-w-[280px] w-full bg-white shadow-2xl z-10 flex flex-col p-6 animate-fade-in-up overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-center pb-4 border-b border-slate-100 mb-6">
              <span className="font-extrabold text-slate-800 text-sm uppercase tracking-wider flex items-center space-x-1.5">
                <HiOutlineAdjustments className="w-5 h-5 text-indigo-500" />
                <span>Filters</span>
              </span>
              <button 
                onClick={() => setShowMobileFilters(false)}
                className="text-xs text-slate-400 hover:text-slate-650 font-bold cursor-pointer"
              >
                Close
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        {/* Left Filters Sidebar (PC Only) */}
        <div className="hidden lg:block lg:col-span-1 lg:sticky lg:top-6 lg:self-start lg:max-h-[calc(100vh-4rem)] lg:overflow-y-auto custom-scrollbar pr-1">
            ...
            <div className="flex justify-between items-center pb-4 border-b border-slate-100">
              ...
              {(selectedTypes.length > 0 || timeSlots.length > 0) && (
                <button 
                  ...
                  className="text-xs text-indigo-600 hover:text-indigo-700 font-bold transition-premium cursor-pointer"
                >
                  Clear All
                </button>
              )}
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {sortedRoutes.map((route) => {
                ...
                const operatorInitials = (route.bus?.name || 'BS').split(' ').map(w => w[0]).join('').substring(0, 2).toUpperCase();

                return (
                  <div 
                    key={route.id} 
                    className="bg-white border border-slate-200/80 hover:border-indigo-400 rounded-[2.5rem] p-6 shadow-sm hover:shadow-md hover:shadow-indigo-500/5 transition-premium flex flex-col md:flex-row justify-between items-stretch animate-fade-in-up"
                  >
                    {/* Bus Info Column */}
                    <div className="flex-grow flex flex-col justify-between space-y-4 md:space-y-0">
                      <div className="flex items-start space-x-4">
                        {/* Operator Avatar Icon */}
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-500 to-indigo-650 flex items-center justify-center text-white text-base font-black shadow-md shadow-indigo-500/15 select-none flex-shrink-0">
                          {operatorInitials}
                        </div>

                        <div>
                          <div className="flex items-center space-x-3 flex-wrap gap-y-1">
                            <h3 className="font-black text-slate-800 text-base">{route.bus?.name || 'Bus Service'}</h3>
                          ...
                          <div className="flex items-center space-x-3 text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-wider">
                            {route.bus?.amenities && route.bus.amenities.toLowerCase().includes('wifi') && (
                              <span className="flex items-center space-x-1 bg-slate-50 px-2.5 py-1 rounded-xl">
                      ...
                      {/* Travel details grid */}
                      <div className="grid grid-cols-3 gap-2 py-4 border-t border-b md:border-t-0 md:border-b-0 border-slate-100 max-w-lg mt-4">
                        <div>
                          <div className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Departure</div>
                          <div className="font-extrabold text-slate-850 text-base mt-1">{formatTime(route.departure_time)}</div>
                          <div className="text-[10px] text-slate-400 font-semibold mt-0.5">{source}</div>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <div className="w-full flex items-center space-x-1 mt-1.5 px-3">
                            <div className="w-2.5 h-2.5 rounded-full border-2 border-emerald-500 bg-white shadow-sm shadow-emerald-500/20"></div>
                            <div className="flex-grow h-0.5 bg-gradient-to-r from-emerald-450 to-indigo-500 relative">
                              <span className="absolute -top-1 left-1/2 -translate-x-1/2 text-xs select-none">🚌</span>
                            </div>
                            <div className="w-2.5 h-2.5 rounded-full border-2 border-indigo-600 bg-indigo-600 shadow-sm shadow-indigo-600/25"></div>
                          </div>
                          <span className="inline-block text-[8px] bg-slate-100 text-slate-500 border border-slate-200/50 font-black tracking-widest uppercase px-2 py-0.5 rounded-full mt-2 select-none">
                            Direct
                          </span>
                        </div>

                        <div className="text-right">
                          <div className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Arrival</div>
                          <div className="font-extrabold text-slate-850 text-base mt-1">{formatTime(route.arrival_time)}</div>
                          <div className="text-[10px] text-slate-400 font-semibold mt-0.5">{destination}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 md:mt-0 md:pl-6 border-t md:border-t-0 md:border-l border-slate-100 flex flex-row md:flex-col justify-between md:justify-center items-center md:items-end space-y-0 md:space-y-4 min-w-[170px]">
                      <div className="text-left md:text-right">
                        <div className="text-[9px] text-slate-400 font-black uppercase tracking-wider">Ticket Price</div>
                      ...
                      <button
                        ...
                        className="bg-indigo-600 hover:bg-indigo-750 text-white font-extrabold text-xs uppercase tracking-wider px-5 py-3.5 rounded-2xl flex items-center space-x-1.5 shadow-md shadow-indigo-600/10 hover:shadow-indigo-650/35 transition-all duration-300 active:scale-95 cursor-pointer hover-glow"
                      >
